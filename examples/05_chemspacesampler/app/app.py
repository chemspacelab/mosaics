...
st.write("📘 README:")
st.write(
    '🔬 This application generates new chemical structures starting from a given molecule. Just enter the parameters below and click "Run ChemSpace Sampler!"'
)

st.write(
    "🌈 Ensemble representation will make distances less noisy. Without it, you may get distances vastly outside of the defined target interval (min_d, max_d). However, it will take longer to run."
)

st.write(
    "⚠️ Note: When the maximal distance is too small, you may get no molecules in the interval, but this also depends on the representation."
)

st.write(
    '🔍 If you want to find the closest molecules, set both distances to 0 and uncheck "RETURN MOLECULES STRICTLY IN THE INTERVAL".'
)


# Parameters input
st.sidebar.image(
    "/app/examples/05_chemspacesampler/app/radar.png",
    caption="Navigate chemical space",
...
smiles = st.sidebar.text_input(
    "Start molecule",
    value="OC1=CC=CC=C1",
    help="Enter the SMILES string of your starting molecule.",
)
selected_descriptor = st.sidebar.selectbox(
    "Select Representation", descriptor_options, help=help_text_selected_descriptor
)
min_d = st.sidebar.number_input(
    "Min distance",
    value=0.0,
    help="Enter the minimal desired distance from the central molecule.",
)
max_d = st.sidebar.number_input(
    "Max distance",
    value=0.0,
    help="Enter the maximal desired distance from the central molecule.",
)
Nsteps = st.sidebar.number_input(
    "#MC steps",
    value=30,
    help="Enter the number of Monte Carlo iterations to be performed.",
)
possible_elements = st.sidebar.multiselect(
    "Select allowed elements in the generated molecules",
    options=["C", "O", "N", "F", "P", "S", "Si", "Br", "Cl", "B"],
    default=["C", "O", "N", "F"],
    help="Enter the elements that are allowed in the generated molecules.",
)
nhatoms_range = st.sidebar.text_input(
    "Number of heavy atoms (non-hydrogen)",
    value="2, 9",
    help="Enter the range of the number of heavy atoms that should be in the generated molecules.",
).split(", ")
synth_cut_soft, synth_cut_hard = st.sidebar.slider(
    "Select soft and hard cutoff for Synthesizability (1 easy to 10 impossible to make) read the (?) for more info",
    min_value=1.0,
    max_value=10.0,
    value=(6.8, 9.5),
    step=0.1,
...
strictly_in = st.sidebar.checkbox(
    "Only return molecules strictly in the distance interval?",
    value=False,
    help="During MC you also accept molecuels outside of the 0 interval if temperature allows, this just affects postprocessing",
)
mmff_check = st.sidebar.checkbox(
    "MMFF94 parameters exist? (another sanity check)",
    value=True,
    help="Check if the generated molecules should have MMFF94 parameters.",
)
ensemble = st.sidebar.checkbox(
    "Ensemble representation (affects only geometry-based representations, BoB & SOAP)",
    value=False,
    help="Check if the ensemble representation should be used. It affects only geometry-based representations (BoB & SOAP).",
)
default_bonds = "(8, 9), (8, 8), (9, 9), (7, 7)"

# Input field for forbidden bonds
bonds_input = st.sidebar.text_input(
    "Enter forbidden bonds as pairs (a, b), separated by commas:",
    value=default_bonds,
    help="Enter forbidden bonds as pairs (a, b), separated by commas where a anb b are the atomic numbers of the atoms forming the bond.",
)


# just add some text to the sidebar
st.sidebar.write(
    "📚 References: Understanding Representations by Exploring Galaxies in Chemical Space, Jan Weinreich, Konstantin Karandashev, Guido von Rudorff arXiv:xxxx.xxxxx"
)

# Convert input string to list of tuples
try:
    forbidden_bonds = [
        tuple(map(int, bond.strip(" ()").split(","))) for bond in bonds_input.split("),")
    ]
    ...
    try:
        ...
        for atom in mol.GetAtoms():
            ...
            if symbol != "H" and symbol not in possible_elements:
                ...
                st.success(
                    f"The starting molecule contains element {symbol} which is not in the allowed elements list. It was therefore added..."
                )
        ...
        if int(nhatoms_range[1]) < len(atom_symbols_no_H):
            ...
            st.success(
                f"The starting molecule contains more heavy atoms than the upper bound of the permitted range. It was therefore updated to {nhatoms_range[1]}..."
            )
            st.write("Updated number of heavy atoms:", nhatoms_range)

        params = chemspace_sampler_default_params.make_params_dict(
            selected_descriptor,
            min_d,
            max_d,
            strictly_in,
        ...
        if len(MOLS) == 0:
            st.error(
                "No molecules found. Try to change the parameters such as increasing the minimal distance or the number of iterations."
            )
            st.stop()
        ...
        if len(ALL_RESULTS) > 4:
            ...
            FP_array = chemspace_potentials.get_all_FP(
                [Chem.MolFromSmiles(smi) for smi in ALL_RESULTS["SMILES"].values],
                nBits=2048,
            )
            ...
            pca_start = pca.transform(
                chemspace_potentials.get_all_FP([Chem.MolFromSmiles(smiles)], nBits=2048)
            )
        ...
        mol_df["img"] = mol_df["SMILES"].apply(
            lambda x: mol_to_img(mol_to_3d(Chem.MolFromSmiles(x)))
        )
        ...
        st.error(
            "An error occurred. Please check your input parameters and try again. \
