...
def extended_get_single_FP(smi, nBits=2048, useFeatures=True):
    ...
    """
    x = ExplicitBitVect_to_NumpyArray(get_single_FP(smi, nBits=nBits, useFeatures=useFeatures))
    return x
...
def get_single_FP(mol, nBits=2048, useFeatures=True):
    ...
    fp_mol = rdMolDescriptors.GetMorganFingerprintAsBitVect(
        mol, radius=4, nBits=nBits, useFeatures=useFeatures
    )
    return fp_mol
        ...
        for j in range(size):
            for k in range(size):
                if threeb[j][k][-1] != 0:
                    ...
                    f2 += (charge * h1 * exponent * (1 + (num2 * angle1 * angle2 * angle3))) / (
                        atm**angular_scaling
                    )
...
def generate_mbdf(
    ...
    """
    assert (
        nuclear_charges.shape[0] == coords.shape[0]
...
class AtomicEnergyRepresentation:
    ...
    def sort_and_add_padding(self, elementwise_vectors, num_atoms):
        for Z in elementwise_vectors.keys():
            ...
            assert (
                len(elementwise_vectors[Z]) <= num_atoms[Z]
            ), f"Number of atoms (={len(elementwise_vectors[Z])}) with Z = {Z} exceeds maximum number of atoms (={num_atoms[Z]})"
            elementwise_vectors[Z] = elementwise_vectors[Z] + (
                num_atoms[Z] - len(elementwise_vectors[Z])
            ) * [0.0]
